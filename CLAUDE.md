...
| State / storage | React state + **localStorage** (small typed wrapper) + **IndexedDB for photo blobs** (`lib/photoStore.ts`) | No backend needed. localStorage holds ~5 MB (UTF-16 counted) and once FILLED UP silently losing edits — photos now live in IndexedDB (hundreds of MB) as `idb:<poiId>` markers; the sync wire format still carries full data-URLs (inflate on push, deflate on pull), so the Worker/KV and older clients are untouched. |
...
Begin Historic Route 66 sign (photo, 15m) · Millennium Park / Cloud Gate (45m) · Navy Pier (1h) · 360 Chicago / John Hancock (1h) · Lincoln Park Zoo (free, 1.5h) · Art Institute of Chicago (2–3h) · **United Center + Michael Jordan statue [nba]** · **Bulls team store [nba]** · End Historic Route 66 sign (photo) · New Route 66 Begin sign near Navy Pier (photo) · Chicago River architecture cruise (~10 AM Aug 4, book ahead, meeting point Michigan/Wacker) · The Chicago Theatre (exterior; interior only via guided tour) · Starbucks Reserve Roastery (optional) · Nutella Cafe (optional) · Alice & Wonder – State (souvenirs)

> **Rental car (July 2026):** pickup = Hertz "Chicago – West Washington Street" on Aug 5; drop-off = **LAX Rental Car Return Center** on Aug 19. Shown as a dedicated "🚗 Rental car" section under the hotel on the Chicago and LA city views (`CAR_RENTAL` in tripData.ts) — deliberately NOT part of the editable stops list (the human's choice).
>
> **Chicago day-by-day plan (July 2026):** a timed walking plan for Aug 3–5 lives in `src/data/chicagoPlan.ts` and renders at `#/more/plan` (`ChicagoPlanView`), with a shortcut card on the Chicago city view. Constraints baked in: arrival ~15:00 Aug 3; river cruise ~10:00 Aug 4 (earliest slot, book ahead); dinner always ~8–9 PM; Lincoln Park = park stroll only (no zoo); Chicago Theatre & Art Institute = exterior only; Aug 5 morning = Hertz pickup then United Center by car (it sits west, on the way out of the city) before hitting I-55 S. The plan is editable (add/edit/remove/reorder/reset) via overrides in `lib/planOverrides.ts`, synced across the phones through the Worker's `/plan` endpoint (`lib/planSync.ts`), same PIN + merge rules as `/itinerary`.
